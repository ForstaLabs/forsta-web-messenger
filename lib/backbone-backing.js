...
(function () {
    ...
    class IDBDriver {
        constructor(schema, ready, onerror) {
            ...
            console.info(`Opening database ${this.schema.id} (v${lastMigrationPathVersion})`);
            this.dbRequest = indexedDB.open(this.schema.id, lastMigrationPathVersion);
        ...
        try {
            ...
            if (e instanceof ifrpc.RemoteError) {
                if (e.remoteError.name === 'NotFound') {
                    ...
                    // Do our best to make it look like the remote error.
                    exc = new Error(e.remoteError.message);
                    Object.assign(exc, e.remoteError);
                }
            }
            if (optionedErrorCallback) {
    ...
    Backbone.initBackingRPCHandlers = function(rpc) {
        rpc.addCommandHandler('backbone-sync', async (method, data, options) => {
            ...
            if (method !== 'read') {
                throw new Error(`RPC backing is readonly: ${method} blocked`);
            }
