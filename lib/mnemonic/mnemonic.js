(function() {
    ...
    ns.Mnemonic = class Mnemonic {
        ...
        static _belongsToWordlist(mnemonic, wordlist) {
            ...
            return words.every(x => wordlist.indexOf(x) !== -1);
        }
         ...
         * @returns {Uint8Array}
         */
        async toSeed(passphrase, iterations) {
            const keyData = (new TextEncoder()).encode(this.phrase.normalize('NFKD'));
            const key = await crypto.subtle.importKey('raw', keyData, {name: 'PBKDF2'}, false, ['deriveBits']);
            const saltPassphrase = ('mnemonic' + (passphrase || '')).normalize('NFKD');
            const salt = (new TextEncoder()).encode(saltPassphrase);
