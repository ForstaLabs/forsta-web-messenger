...
(function () {
    ...
    AccountManager.prototype.extend({
        ...
        registerSingleDevice: function(phone, verificationCode) {
            ...
            return libsignal.KeyHelper.generateIdentityKeyPair().then(function(identityKeyPair) {
                return createAccount(phone, verificationCode, identityKeyPair).
                    then(generateKeys).
                    then(registerKeys).
        ...
        registerDevice: async function(addr, password, deviceName) {
            ...
            const keys = await this.generateKeys(20);
            await this.server.registerKeys(keys);
            this.registrationDone();
        },

        registerSecondDevice: function(setProvisioningUrl, confirmPhone, progressCallback) {
            var registerKeys = this.server.registerKeys.bind(this.server);
            var createAccount = this.createAccount.bind(this);
                        ...
                        handleRequest: function(request) {
                            if (request.path === "/v1/address" && request.verb === "PUT") {
                                ...
                                resolve(provisioningCipher.decrypt(envelope).then(function(provisionMessage) {
                                    return confirmPhone(provisionMessage.addr).then(function(deviceName) {
