...
(function (root, factory) {
    ...
    function Driver(schema, ready, onerror) {
        ...
        this.dbRequest.onsuccess = ev => {
            this.db = ev.target.result; // Attach the connection ot the queue.
    ...
    function sync(method, object, options) {
        if (method === "closeall"){
            ...
            Databases = {};
            return new Promise(res => res()); // immediately done
        }
        ...
        return new Promise((resolve, reject) => {
            var success_save = options.success;
            ...
            var error_save = options.error;
            options.error = function(resp) {
                error_save && error_save(resp);
