...
;(function () {
    ...
    function AccountManager(url, port, username, password) {
        this.server = new TextSecureServer(url, port, username, password);
    }
        ...
        createAccount: function(number, verificationCode, identityKeyPair, deviceName, userAgent) {
            ...
            return this.server.confirmCode(
                ...
                return textsecure.storage.protocol.clearSessionStore().then(function() {
                    ...
                    textsecure.storage.put('identityPubKey', identityKeyPair.pubKey);
                    textsecure.storage.put('identityPrivKey', identityKeyPair.privKey);
                ...
                for (var keyId = startId; keyId < startId+count; ++keyId) {
                    promises.push(
                        libsignal.KeyHelper.generatePreKey(keyId).then(function(res) {
                            ...
                            if (progressCallback) { progressCallback(keyId - startId); }
                        })
                    );
                }
