...
(function () {
    ...
    class RPCInterface {
        ...
        async dbCommand(method, options) {
            return await F.parentRPC.invokeCommand(`db-gateway-${method}-${this.schema.id}`, options);
        }
        ...
        async create(storeName, model) {
            ...
            return await this.dbCommand('create', {
                storeName,
                json,
                idAttribute,
                idFallback,
        ...
        async update(storeName, model) {
            ...
            return await this.dbCommand('update', {
                storeName,
                json,
                idAttribute
            });
        ...
        async read(storeName, model, index, notFoundOkay) {
            ...
            const result = await this.dbCommand('read', {
                storeName,
                json,
                idAttribute,
                index,
        ...
        async delete(storeName, model) {
            ...
            const json = model.toJSON();
            const idAttribute = _.result(model, 'idAttribute');
            return await this.dbCommand('delete', {
                storeName,
                json,
                idAttribute
            });
        ...
        async clear(storeName) {
            ...
            return await this.dbCommand('clear', {storeName});
        }
        ...
        async query(storeName, collection, options) {
            ...
            const results = [];
            do {
                const batch = await this.dbCommand('query', {
                    storeName,
                    idAttribute,
                    offset,
                    index: options.index,
                ...
                if (!batch.length) {
                    ...
                    offset += batch.length;
                    results.push.apply(results, batch.filter(options.filter));
                } else {
                    results.push.apply(results, batch);
                }
            } while(hasFilter && (options.limit && results.length < options.limit));
            return results.slice(0, options.limit); // Trim overflow from filter batchs.
        }
    }
    ...
    Backbone.sync = function(method, storable, options) {
        ...
        if (!storable || !storable.database) {
            ...
            sync = syncRPC;
            delete self.indexedDB; // Prevent leaks by disabling native API.
        } else {
            sync = syncIDB;
        }
