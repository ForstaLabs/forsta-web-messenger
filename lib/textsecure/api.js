...
(function() {
    ...
    ns.TextSecureServer.prototype = {
        ...
        _fetch: async function(param) {
            ...
            if (!resp.ok) {
                const e = HTTPError(resp.status, await resp.text());
                if (HTTP_MESSAGES.hasOwnProperty(e.code)) {
                    e.message = HTTP_MESSAGE[e.code];
                } else {
            ...
            if (resp.status !== 204) {
                ...
                if (param.validateResponse &&
                    !validateResponse(data, param.validateResponse)) {
                    throw HTTPError(resp.status, data);
                }
                return data;
            }
        },
