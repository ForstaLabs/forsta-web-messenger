...
(function() {
    ...
    ns.TextSecureServer.prototype = {
        ...
        fetch: async function(param) {
            ...
            const username = param.username || this.username;
            const password = param.password || this.password;
            ...
            const method = param.httpType || 'GET';
            let resp;
            try {
                resp = await fetch(`${this.url}/${path}`, {method, body, headers});
            } catch(e) {
        ...
        getDevices: async function() {
            const data = await this.fetch({call: 'devices'});
