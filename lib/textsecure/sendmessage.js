...
class Message {
    ...
    toProto() {
        if (this._protofied) {
            debugger;
            throw new Error("Rand twice! XXX");
        } else {
            this._protofied = true; // XXX testing
        }
        ...
        data.attachments = this.attachmentPointers; // XXX Can be optional?
        if (this.flags) {
...
MessageSender.prototype = {
    ...
    tryMessageAgain: async function(addr, encodedMessage, timestamp) {
        ...
        return await this.sendMessageProto(timestamp, [addr], content);
    },
    ...
    sendMessage: async function(attrs) {
        ...
        try {
            return await this.sendMessageProto(m.timestamp, m.recipients, m.toProto());
        } finally {
    ...
    sendMessageProto: async function(timestamp, addrs, msgproto) {
        ...
        const p = new Promise((resolve, reject) => {
            outmsg = new OutgoingMessage(this.server, timestamp, addrs, msgproto, res => {
                res.content = msgproto.toArrayBuffer();
    ...
    sendSyncMessage: async function(encodedContent, timestamp, destination, expirationStartTimestamp) {
        if (this.server.deviceId == 1) {
            // XXX suspect...
            console.warn("NOT Skipping Sync Message because I am deviceId 1");
            //return;
        }
        const content = textsecure.protobuf.Content.decode(encodedContent);
        ...
        return await this.sendMessageProto(Date.now(), [this.server.addr], syncContent);
    },
    ...
    sendRequestGroupSyncMessage: async function() {
        ...
        return await this.sendMessageProto(Date.now(), [this.server.addr], content);
    },
    ...
    sendGroupProto: async function(addrs, content, timestamp) {
        ...
        return await this.sendMessageProto(timestamp, addrs, content);
    },
    ...
    closeSession: async function(addr, timestamp) {
        ...
        const res = await this.sendMessageProto(timestamp, [addr], content);
    ...
    createGroup: async function(addrs, name, avatar) {
        ...
        data.group.id = stringToArrayBuffer(group.id);
        const allAddrs = group.get('addrs'); // Includes our own address.
        data.group.type = textsecure.protobuf.GroupContext.Type.UPDATE;
        data.group.members = allAddrs;
        data.group.name = name;
        if (avatar) {
            data.group.avatar = await this.makeAttachmentPointer(avatar);
        }
        await this.sendGroupProto(allAddrs, content);
        return group.id;
    },
    ...
    addAddrToGroup: async function(id, addr) {
        ...
        addrs.push(addr);
        return await this.updateGroup(id, {recipients: addrs});
    },
