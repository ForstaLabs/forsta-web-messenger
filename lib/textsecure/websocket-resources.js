    ...
    class WebSocketResource {
        ...
        async onMessage(encodedMsg) {
            ...
            if (message.type === textsecure.protobuf.WebSocketMessage.Type.REQUEST) {
                ...
                if (this.outgoingRequests.has(response.id)) {
                    const request = this.outgoingRequests.get(response.id);
                    this.outgoingRequests.delete(response.id);
                    ...
                    throw new ReferenceError('Received response for unknown request ' +
                                             message.response.id);
                }
            } else {
