...
(function () {
    ...
    AccountManager.prototype.extend({
        ...
        _generateDeviceInfo: function(identityKeyPair, addr, name) {
        ...
        registerAccount: async function(addr, name) {
        ...
        registerDevice: async function(setProvisioningUrl, confirmAddress, progressCallback) {
                ...
                const wsr = new WebSocketResource(ws, {
                    ...
                    handleRequest: request => {
                        if (request.path === "/v1/address" && request.verb === "PUT") {
                            ...
                            const msgEnvelope = textsecure.protobuf.ProvisionEnvelope.decode(request.body, 'binary');
                            request.respond(200, 'OK');
                            wsr.close();
                            resolve(msgEnvelope);
                        } else {
        ...
        refreshPreKeys: async function() {
            ...
            const lastResortKey = await textsecure.store.loadPreKey(lastResortKeyId);
