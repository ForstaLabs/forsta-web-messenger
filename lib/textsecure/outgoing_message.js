...
function OutgoingMessage(server, timestamp, addrs, message, callback) {
    ...
    const legacy = message instanceof textsecure.protobuf.DataMessage;
    if (legacy) {
        debugger;
        throw new Error("Killing legacy XXX");
    } 
...
OutgoingMessage.prototype = {
    ...
    getStaleDeviceIdsForAddr: async function(addr) {
        ...
        if (deviceIds.length === 0) {
            return [1]; // XXX I think signal server believes device id is required.  This is bad.
        }
        const noSession = [];
        await Promise.all(deviceIds.map(async function(id) {
