...
MessageSender.prototype = {
    ...
    createGroup: async function(numbers, name, avatar) {
        ...
        const proto = new textsecure.protobuf.DataMessage();
        proto.group = new textsecure.protobuf.GroupContext();
        const group = await textsecure.store.createGroup(numbers);
        proto.group.id = stringToArrayBuffer(group.id);
        const allNumbers = group.get('numbers'); // Includes our own number.
        proto.group.type = textsecure.protobuf.GroupContext.Type.UPDATE;
        proto.group.members = allNumbers;
        proto.group.name = name;
        if (avatar) {
            const attachment = await this.makeAttachmentPointer(avatar);
            proto.group.avatar = attachment;
        }
        await this.sendGroupProto(allNumbers, proto);
        return proto.group.id;
    },

    updateGroup: async function(id, name, avatar, numbers) {
        console.assert(numbers instanceof Array);
        const proto = new textsecure.protobuf.DataMessage();
        proto.group = new textsecure.protobuf.GroupContext();
        ...
        proto.group.type = textsecure.protobuf.GroupContext.Type.UPDATE;
        proto.group.name = name;
        await textsecure.store.updateGroupNumbers(id, numbers);
