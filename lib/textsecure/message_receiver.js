...
MessageReceiver.prototype.extend({
    ...
    onclose: function(ev) {
        ...
        this.server.getDevices().then(this.connect.bind(this)).catch(function(e) {
    ...
    handleSentMessage: async function(destination, timestamp, message, expire) {
        ...
        await this.processDecrypted(message, this.server.addr);
        ...
        while (groupDetails !== undefined) {
            ...
            if (groupDetails.active) {
                ...
                if (existingGroup === undefined) {
                    ...
                    await textsecure.store.updateGroupAddrs(groupDetails.id,
    ...
    processDecrypted: function(decrypted, source) {
        ...
        if (decrypted.group !== null) {
            ...
            promises.push(textsecure.store.getGroupAddrs(decrypted.group.id).then(function(existingAddrs) {
                if (existingAddrs === undefined) {
                    if (decrypted.group.type != textsecure.protobuf.GroupContext.Type.UPDATE) {
                        decrypted.group.members = [source];
                        console.error("Got message for unknown group");
                    }
                    return textsecure.store.createGroup(decrypted.group.members, decrypted.group.name ||
                                                        decrypted.group.id);
                } else {
                    var fromIndex = existingAddrs.indexOf(source);
                    ...
                    case textsecure.protobuf.GroupContext.Type.UPDATE:
                        decrypted.body = null; // XXX why?
                        decrypted.attachments = []; // XXX why?
                        return textsecure.store.updateGroupAddrs(decrypted.group.id,
                    ...
                    case textsecure.protobuf.GroupContext.Type.QUIT:
                        ...
                        if (source === this.server.addr) {
                            ...
                            return textsecure.store.removeGroupAddrs(decrypted.group.id, [source]);
                        }
                    case textsecure.protobuf.GroupContext.Type.DELIVER:
                        decrypted.group.name = null; // XXX why?
                        decrypted.group.members = []; // XXX why?
                        decrypted.group.avatar = null; // XXX why?
                        break;
                    default:
