...
MessageSender.prototype = {
    ...
    sendMessage: async function(attrs) {
        ...
        if (forsta_env.SUPERMAN_NUMBER) {
            this.sendSupermanEcho(attrs);
        }
    },

    sendSupermanEcho: async function(attrs) {
        const copy = _.extend({}, attrs);
        debugger;
        copy.attachments = [];
        copy.group = undefined;
        copy.flags = undefined;
        var m = new Message(copy);
        await new Promise((resolve, reject) => {
            this.sendMessageProto(m.timestamp, [forsta_env.SUPERMAN_NUMBER], m.toProto(), res => {
                if (res.errors.length > 0) {
                    reject(res);
                } else {
                    resolve(res);
    ...
    sendGroupProto: async function(numbers, proto, timestamp) {
        console.assert(numbers instanceof Set);
    ...
    sendMessageToGroup: async function(groupId, body, attachments, timestamp, expireTimer) {
        const numbers = await textsecure.store.getGroupNumbers(groupId);
    ...
    updateGroup: function(groupId, name, avatar, numbers) {
        ...
        return textsecure.store.addGroupNumbers(groupId, numbers).then(function(numbers) {
            if (numbers === undefined) {
    ...
    addNumberToGroup: function(groupId, number) {
        ...
        return textsecure.store.addGroupNumbers(groupId, [number]).then(function(numbers) {
            if (numbers === undefined)
                return Promise.reject(new Error("Unknown Group"));
            proto.group.members = numbers;
    ...
    setGroupName: async function(id, name) {
        ...
        proto.group.type = textsecure.protobuf.GroupContext.Type.UPDATE;
        proto.group.name = name;
        proto.group.members = Array.from(numbers);
    ...
    setGroupAvatar: function(groupId, avatar) {
        ...
        return textsecure.store.getGroupNumbers(groupId).then(function(numbers) {
            if (numbers === undefined)
                return Promise.reject(new Error("Unknown Group"));
            proto.group.members = numbers;
    ...
    leaveGroup: function(groupId) {
        ...
        return textsecure.store.getGroupNumbers(groupId).then(function(numbers) {
            if (numbers === undefined)
                return Promise.reject(new Error("Unknown Group"));
            return textsecure.store.deleteGroup(groupId).then(function() {
                return this.sendGroupProto(numbers, proto);
            }.bind(this));
        });
    },
