...
MessageReceiver.prototype.extend({
    ...
    handleDataMessage: async function(message, envelope) {
    ...
    handleContentMessage: async function(envelope) {
        ...
        if (content.syncMessage) {
            await this.handleSyncMessage(content.syncMessage, envelope);
        } else if (content.dataMessage) {
            await this.handleDataMessage(content.dataMessage, envelope);
        } else {
            throw new Error('Got content message with no dataMessage or syncMessage');
        }
    },

    handleSyncMessage: async function(message, envelope) {
        ...
        if (message.sent) {
            ...
            await this.handleGroups(message.groups, envelope);
        } else if (message.blocked) {
            this.handleBlocked(message.blocked, envelope);
        } else if (message.request) {
            console.warn('Unimplemented SyncMessage Request', message);
        } else {
