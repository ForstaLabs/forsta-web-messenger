...
(function () {
                        ...
                        if (options.abort || (options.limit && processed >= options.limit)) {
                            ...
                            if (!options.filter || typeof options.filter !== 'function' || options.filter(cursor.value)) {
                                ...
                                if (options.addIndividually) {
                                    throw new Error("addIndividually not supported");
                                } else if (options.clear) {
                                    throw new Error("clear not supported because it's bad");
                                } else {
    ...
    class RPCInterface {
        ...
        async init() {
            ...
            await F.parentRPC.invokeCommand('db-gateway-init', this.schema.name, this.schema.id, this.version);
            const readyEvent = new Event('dbready');
        ...
        async execute(method, storable, options) {
            const storeName = options.storeName || storable.storeName;
            if (method === 'create') {
                await this.create(storeName, storable, options);
            } else if (method === 'read') {
                if (storable.id || storable.cid) {
                    await this.read(storeName, storable, options);
                } else {
                    await this.query(storeName, storable, options);
                }
            } else if (method === 'update') {
                await this.update(storeName, storable, options);
            } else if (method === 'delete') {
                if (storable.id || storable.cid) {
                    await this.delete(storeName, storable, options);
                } else {
                    assertCollection(storable);
                    await this.clear(storeName, options);
                }
            } else if (method === 'noop') {
        ...
        async update(storeName, model, options) {
            if (this.schema.readonly) {
                throw new Error("Database is readonly");
            }
            assertModel(model);
            ...
            try {
                await F.parentRPC.invokeCommand(`db-gateway-update-${this.schema.id}`, {
                    storeName,
                    json,
                    idAttribute
                });
        ...
        async read(storeName, model, options) {
            assertModel(model);
            const json = model.toJSON();
            const idAttribute = _.result(model, 'idAttribute');
            let keyIdent;
            ...
            if (!result && options.not_found_error !== false) {
                options.error(new NotFound(keyIdent));
            } else {
        ...
        async query(storeName, collection, options) {
            ...
            try {
                const result = await F.parentRPC.invokeCommand(`db-gateway-query-${this.schema.id}`, {
                    ...
                    clear: options.clear, // XXX
                    abort: options.abort, // XXX
                    hasFilter,
                    filterSig
                });
    ...
    async function syncRPC(method, storable, options) {
        ...
        return await new Promise(async (resolve, reject) => {
            ...
            const schema = storable.database;
            if (!rpcInterfaces.has(schema.id)) {
                const rpcInterface = new RPCInterface(schema);
            ...
            try {
                await rpcInterfaces.get(schema.id).execute(method, storable, options);
            } catch(e) {
                reject(e);
            }
        });
