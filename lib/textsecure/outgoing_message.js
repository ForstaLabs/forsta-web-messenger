...
OutgoingMessage.prototype = {
    ...
    registerError: function(number, reason, error) {
        if (!error || error.name === 'HTTPError' && error.code !== 404) {
            error = new textsecure.OutgoingMessageError(number, this.message.toArrayBuffer(),
    ...
    reloadDevicesAndSend: async function(number, recurse) {
        ...
        return await this.doSendMessage(number, deviceIds, recurse);
    },
    ...
    sendToNumber: async function(number) {
